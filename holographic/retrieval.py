...
class FactRetriever:
    ...
    def _fts_candidates(
        ...
        with BM25 scoring. Normalizes BM25 score to [0, 1] range.
        """
        conn = self.store._conn
        ...
        sql = f"""
            SELECT f.*, bm25(facts_fts) as fts_bm25
            FROM facts_fts
        ...
        # BM25: negative values, lower = better; normalize to [0,1]
        raw = [row["fts_bm25"] for row in rows]
        min_bm25 = min(raw) if raw else 0.0
        max_bm25 = max(raw) if raw else 1.0
        span = max(max_bm25 - min_bm25, 1e-6)
        results = []
        for row in rows:
            fact = dict(row)
            fact.pop("fts_bm25", None)
            fact["fts_rank"] = 1.0 if span < 1 else (max_bm25 - row["fts_bm25"]) / span  # invert: lower bm25 = better; ties → 1.0
            results.append(fact)
